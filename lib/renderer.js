...
default_rules.math_block = function (tokens, idx, options, env, slf) {
  ...
  var token = tokens[idx];
  var html = katex.renderToString(token.content, {
    throwOnError: true,
    displayMode: true,
    macros: env.katex_macros,
    globalGroup: true
  });
  return '<div class="markup-it-math-block">' + html + '</div>';
};
...
default_rules.math_inline = function (tokens, idx, options, env) {
  ...
  return '<span class="markup-it-math-inline">' + html + '</span>';
}
...
// Code for math in mathjax

// default_rules.blockmath = function (tokens, idx, options, env, slf) {
//   var token = tokens[idx];

//   const node = html.convert(token.content || '', {
//     display: true,
//     em: 16,
//     ex: 8,
...
//   var result = adaptor.outerHTML(node);
